...
//   onDataProcessed,  // callback to parent after backend preprocess
//   isProcessing,
//   dataPreview,
//   headerRowIndex,
//   setHeaderRowIndex,
...
//       setShowModal(true);    // open modal for header row selection
//     };
...
//   const headerOptions = (dataPreview || []).slice(0, 5).map((_, index) => (
//     <option key={index} value={index}>
//       Row {index + 1} (Index {index})
//     </option>
//   ));

//   const maxCols = dataPreview?.reduce((max, row) => Math.max(max, row.length), 0);
...
//       >
//         <input type="file" accept=".csv" onChange={handleFileChange} className="hidden" ref={inputRef} />
//         <Upload className="mx-auto h-12 w-12 text-gray-400" />
//         <p className="mt-1 text-sm text-gray-600">Drag & drop CSV here or click to select.</p>
//         {file && <p className="mt-2 text-indigo-500 font-medium">{file.name}</p>}
//       </div>
...
//               ⚠️ Make sure to correctly select the header row — it affects AI interpretation.
//             </p>
...
//                     className="form-radio text-indigo-600 border-gray-300 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600"
//                   />
...
//                       >
//                         {dataPreview[headerRowIndex]?.[colIndex] || `Col ${colIndex + 1}`}
//                       </th>
//                     ))}
//                   </tr>
//                 </thead>
...
//                 className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2 cursor-pointer"
//               >
//                 <Send className="w-4 h-4" /> {isProcessing ? "Processing..." : "Preprocess CSV"}
//               </button>
//             </div>
//           </div>
//         </div>
...
  const handleFileChange = (e) => {
    const selectedFile = e.target.files[0];
    // IMPORTANT: DO NOT USE alert(). Replace with custom modal or console.error.
    if (!selectedFile || !selectedFile.name.endsWith(".csv")) {
...
      });
      // IMPORTANT: DO NOT USE alert().
      console.log("✅ CSV preprocessing done! You can now ask questions.");
      setShowModal(false);
    } else {
      // IMPORTANT: DO NOT USE alert().
...
    console.error("Preprocess error:", err);
    // IMPORTANT: DO NOT USE alert().
    console.error("Failed to preprocess CSV.");
  }
};
...
            {/* Preprocess button - SPINNER ADDED HERE */}
            <div className="mt-4 flex justify-end">
              <button
                onClick={handlePreprocess}
                disabled={isProcessing}
                className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2 cursor-pointer transition duration-150"
              >
                {isProcessing ? (
